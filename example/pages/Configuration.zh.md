...
**`custom`** —— 框架原样保留的公开业务配置。组件内通过 `useSiteConfig().config.custom` 读取，`utils.ts` 等普通运行时模块通过 `getSiteConfig().custom` 读取。可用不同的配置文件设置本地、生产等环境的 API 基础地址。这些值会进入客户端代码，不能存放密钥。
